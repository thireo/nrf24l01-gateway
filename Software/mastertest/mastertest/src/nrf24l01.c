...
void NRF_Enter_RX_Mode(char Addr[])
{
	...
	/*Addr[4] = 0xA2;
	NRF_Write(NRF_W_REGISTER(NRF_RX_ADDR_P2_REG),5, Addr);
	Addr[4] = 0xA3;
	NRF_Write(NRF_W_REGISTER(NRF_RX_ADDR_P3_REG),5, Addr);
	Addr[4] = 0xA4;
	...
	/*NRF_Write_2(NRF_W_REGISTER(NRF_DYNPD_REG),(NRF_DYNPD_DPL_P2));
	NRF_Write_2(NRF_W_REGISTER(NRF_DYNPD_REG),(NRF_DYNPD_DPL_P3));
...
uint32_t NRF_TX_data(uint32_t *data, uint32_t length)
{
	if (length > 12)
	{
		return NRF_FAILURE;
	...
	//char Package_Data[4] = {(char)((Type>>8)&0xFF),(char)((Type>>0)&0xFF),(char)((Command>>8)&0xFF),(char)((Command>>0)&0xFF)};
	//char Package_Data[16] = {0x00,0x00,0x00,0x05,Count,0x00,0x00,0x00,0x01,(char)(Type&0x00),(char)((Command>>24)&0xFF),(char)((Command>>16)&0xFF)}(char)((Command>>8)&0xFF),(char)((Command>>0)&0xFF),0x2A,0x42};
	char Package_Data[16] = {0x2A,0x42,0x05,Count,0x00,0x00,0x00,0x01,0x2A,0x42};
